...
GB = GenzBretz(maxpts = 5e4, abseps = 1e-9, releps = 0)
  ...
  while(criterio > error){
    ...
    for (j in 1:m ){
      ...
      if(sum(cc1)==0){
        ...
        #ver[j,]<- dmt(as.vector(y1),as.vector(gammai),Psi,df=nu)
      ...
      if(sum(cc1)>0){
         ...
         if(sum(cc1)==nj[j]){
          ...
          auxupper<-y1-muUi
          auxU1<-pmvt(upper = c(auxupper), sigma = as.matrix(SigmaUiA), df = nu+2,algorithm = GB)[1]
          auxU2<-pmvt(upper = c(auxupper), sigma = as.matrix(SigmaUi), df = nu,algorithm = GB) [1]
          ...
          ver[j,]<-pmvt(upper = c(auxupper), sigma = as.matrix(SigmaUi), df = nu,algorithm = GB) [1]
          #ver[j,]<-(pmt(y1,muUi,SigmaUi,df=nu))
          ...
          auxU1<-pmvt(upper = c(auxupper), sigma = as.matrix(SigmaUiA), df = nu1+2, algorithm = GB)[1]
          auxU2<-pmvt(upper = c(auxupper), sigma = as.matrix(SigmaUi), df = nu1, algorithm = GB)[1]
          ...
          #SigmaUiAx<-SigmaUi*nu/(nu+2)

          #auxU1x<-pmvt(upper = c(auxupper), sigma = SigmaUiAx, df = nu1+2)[1]
          #auxU2x<-auxU2

          tu<-U0#as.numeric(auxU1x/auxU2x)
          ...
          #ver[j,]<-dmt(y1[cc1==0],gammai[cc1==0],Psi[cc1==0,cc1==0],df=nu)*(pmt(y1[cc1==1],muUi,SigmaUi,df=nu1))
      ...
      soma5<- soma5 + (((nu+nj[j])/(nu+nj[j]+2))*t(x1)%*%solve(Psi)%*%x1-t(x1)%*%solve(Psi)%*%(tuyy-tuy%*%t(tuy))%*%solve(Psi)%*%x1)
      ...
      tui[j]<-tu
      tyi[(sum(nj[1:j-1])+1) : (sum(nj[1:j])),j]<-ty
    }
  ...
  if(diagnostic)
     obj.out <- c(list(beta = beta1, sigmae= sigmae, dd = dd, crit, iter = count, ubi = tubi, 
                  ubbi = tubbi, uybi = tuybi, uyi = tuyi, uyyi = tuyyi, ui = tui, yi = tyi, diagnostic = diagnostic),crit)
  else
